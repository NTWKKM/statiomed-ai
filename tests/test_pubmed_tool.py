...
def test_pubmed_tool_forward_smolagents_call():
    tool = PubMedEvidenceTool()
    ...
    mock_summary_resp.json.return_value = {
        "result": {
            "12345678": {
                "title": "Clinical Efficacy of SGLT2 Inhibitors in Heart Failure.",
                "authors": [{"name": "Smith J"}, {"name": "Taylor R"}],
                "source": "N Engl J Med",
                "pubdate": "2023",
                "articleids": [{"idtype": "doi", "value": "10.1056/NEJMoa2300000"}],
            }
    ...
    with patch("requests.get", side_effect=[mock_search_resp, mock_summary_resp]):
        output = tool.forward(query="SGLT2 heart failure", max_results=1)
        assert "PMID: 12345678" in output
        assert (
            "Citation: Smith J, Taylor R. Clinical Efficacy of SGLT2 Inhibitors in Heart Failure"
            in output
        )
        assert tool.name == "pubmed_evidence_search"
